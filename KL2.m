function [kl2, specEntropy, f, t, treal] = KL2(x, wlen, hop, nfft, fs)
...
for col = 1:coln
    ...
    if(col > 1)
        ...
        klab = sum (  temp1 .* (log2(temp1) - log2(specEntropy(:,col)))  );
        klba = sum (  specEntropy(:,col) .* (log2(specEntropy(:,col)) - log2(temp1) ));
        kl2(1,col) = klab + klba;
        
    end
    
    
    temp1 = specEntropy(:,col);
    % update the index
    indx = indx + hop;
end
